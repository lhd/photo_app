...
// This file is automatically included by javascript_include_tag :defaults
		...
		$(document).ready( function() {
			...
			$('#imagelabel').click(function() {
				$('#searchimages').attr('checked', true);
				$('.children').show();
			});
			$('#userlabel').click(function() {
				$('#searchusers').attr('checked', true);
				$('.children').show();
			});
						
			/* used on images/index.html for sorting */
			$( "#sortable" ).sortable({
			update: function() { 
				var order = $( "#sortable" ).sortable('serialize');
				$.post('/image_orders/', order + "&page=" + page); // <%= @page %>
			}
			});
		});
		
/*
 * This is from jquery-focus plugin
 */
